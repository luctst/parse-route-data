      ...
      if (hasKey) {
        ...
        if (hasOwnProperty(dataFromManager, "itemsSchema")) {
          ...
          return paramsKeys.every((key) => {
            if (Array.isArray(dataFromManager.itemsSchema[key].type)) {
              return dataFromManager.itemsSchema[key].type.every((constructor) => {
                return this[getProtoName(constructor)](true, dataFromRequest[key], dataFromManager.itemsSchema[key])
              })
            }

            return this[getProtoName(dataFromManager.itemsSchema[key].type)](
              true,
              dataFromRequest[key],
