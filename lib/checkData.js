      ...
      if (hasKey) {
        ...
        if (hasOwnProperty(dataFromManager, "itemsType")) {
          ...
          return dataFromRequest.every((item) => {
            ...
            const protoItemsType = getProtoName(dataFromManager.itemsType);
            const itemProto = getProtoName(item);

            if (protoItemsType === "string" || protoItemsType === "objectid") {
              if (itemProto !== "string") {
                msgError = "should be same data type than itemsType field";
                return false;
              }

              if (protoItemsType === "objectid" && !isValidObjectId(item)) {
                msgError = "should be same data type than itemsType field";
                return false;
              }
