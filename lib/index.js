      ...
      if (dataRoute) {
        ...
        const ok = Object.keys(dataRoute).every((key) => {
          ...
          if (Array.isArray(data.type)) {
            const innerTypeError = data.type.every((tp) => typesEnabled.includes(tp));
            if (!innerTypeError) {
            ...
            const protoTypeToCheck = data.type.find((construstorType) => getProtoName(construstorType) === getProtoName(dataFromRequest[key]));

            if (!protoTypeToCheck) {
              const constructorArrayFormated = data.type.map((constructorType) => getProtoName(constructorType));
              dataError = `${key} type is incorrect, received ${getProtoName(dataFromRequest[key])} expected ${constructorArrayFormated.join(' or ')}`;
              return false;
            }
